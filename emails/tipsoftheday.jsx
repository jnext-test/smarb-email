...
const TipRow = ({ key, date, time, distance, runner, race, isLastItem }) => (
  <>
    <Section
      style={isLastItem ? tipInnerRowContainerLast : tipInnerRowContainer}
      key={key}
    >
      <Section style={fixtureRowContainer}>
        <Text style={dateHeader}>
        ...
        <Row style={{ marginBottom: "10px" }}>
          <Column style={{ width: "70%" }}>
            <Row>
              <Column style={{ verticalAlign: "middle", textAlign: "left" }}>
                <Link href={runner?.redirectUrl} style={gamblingLink}>
                  <Text style={runnerName}>
                    {race?.runnerNumber}. {race?.runnerName} (
                    {race?.barrierNumber}){" "}
                  </Text>
                </Link>
                <Row style={{ verticalAlign: "baseline" }}>
          ...
          <Column style={{ width: "30%", textAlign: "right" }}>
            <div style={{ width: "100px", marginLeft: "auto" }}>
              <div style={bookmakerLabelColumn}>
                <Text style={bookmakerLabelText}>
                  {race?.bookmaker || "BOOKMAKER"}
                </Text>
              </div>
              <div style={bookmakerOdds}>
                <Text style={mainOddsValueStyle}>{race?.maxOdd || "5.00"}</Text>
              </div>
            </div>
          </Column>
        </Row>
      ...
      {/* Venue Info */}
      <Link href={race?.redirectUrl}>
        <Row style={venueStyle}>
          <Column style={{ width: "calc(100% - 27px)" }}>
            <Text
              style={venueText}
            >{`R${race?.raceNumber || "1"} ${race?.TrackName || "Track Name"}`}</Text>
          </Column>
          <Column style={{ width: "27px", textAlign: "right" }}>
            <Text style={arrowText}>
              <Img
...
const TipsOfTheDayEmail = () => {
  ...
  const bet = emailResponseData?.raceData?.tipsData;
  ...
  return (
    <Html>
      ...
      <Preview>
        Your SmartB Tip of the Day from the EXPERTS for {currentDate} is right
        here!
      </Preview>
      <Body style={main}>
        <Container style={container}>
          ...
          <Section style={section}>
            ...
            <Text style={para16}>
              Your{" "}
              <span style={{ fontWeight: "bold" }}>SmartB Tip of the Day</span>{" "}
              from the <span style={{ fontWeight: "bold" }}>EXPERTS</span> for{" "}
            ...
            <Section style={tipRowContainer}>
              {raceData.resultsData?.map((race, index) => (
                <TipRow
                  ...
                  date={new Date(race.startDate).toLocaleDateString("en-AU", {
                    weekday: "long",
                    day: "numeric",
                    month: "numeric",
                    year: "numeric",
                  ...
                  race={race}
                  isLastItem={index === raceData.resultsData.length - 1}
                />
              ))}
            </Section>
          </Section>
...
const tipInnerRowContainer = {
  borderBottom: "1px solid #E8EAEC",
  paddingBottom: "18px",
  marginBottom: "20px",
};

const tipInnerRowContainerLast = {
  paddingBottom: "0",
  marginBottom: "0",
  borderBottom: "none",
};
